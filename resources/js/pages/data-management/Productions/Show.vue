...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    ...
    { title: props.production?.production_name ?? t('data_management.show'), href: window.location.pathname },
]);
...
const confirmDelete = () => {
    if (!confirm(t('data_management.confirm_delete_production'))) {
        return;
    }
...
<template>
    <Head :title="`${t('data_management.production')} — ${props.production.production_name}`" />
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-4">
            <div class="flex items-start justify-between gap-4 mb-6">
                <div>
                    ...
                    <p class="mt-1 text-sm text-muted-foreground dark:text-muted-foreground">
                        {{ t('data_management.status') }}: <strong class="capitalize">{{ props.production.status }}</strong>
                    </p>
                    <p class="mt-2 text-sm text-muted-foreground dark:text-muted-foreground">
                        {{ t('data_management.machine_groups') }}: <strong>{{ props.production.process_count }}</strong>
                    </p>
                </div>
            ...
            <div class="mt-6 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border/70 bg-card dark:bg-card p-6">
                <h3 class="text-lg font-medium mb-4">{{ t('data_management.production_details') }}</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                        <span class="text-muted-foreground dark:text-muted-foreground">{{ t('data_management.id') }}:</span>
                        <span class="ml-2 font-medium">{{ props.production.production_id }}</span>
                    </div>
                    <div>
                        <span class="text-muted-foreground dark:text-muted-foreground">{{ t('data_management.name') }}:</span>
                        <span class="ml-2 font-medium">{{ props.production.production_name }}</span>
                    </div>
                    <div>
                        <span class="text-muted-foreground dark:text-muted-foreground">{{ t('data_management.status') }}:</span>
            ...
            <div class="mt-6 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border/70 bg-card dark:bg-card p-6">
                <h3 class="text-lg font-medium mb-4">{{ t('data_management.attached_machine_groups') }}</h3>
                <p class="text-sm text-muted-foreground dark:text-muted-foreground mb-4">
                    {{ t('data_management.attached_machine_groups_description') }}
                </p>
                ...
                <div v-if="!props.production.machine_groups || props.production.machine_groups.length === 0" 
                    ...
                    {{ t('data_management.no_machine_groups_attached') }}
                </div>
                ...
                <div v-else class="space-y-6">
                    ...
                    >
                            ...
                            <div class="flex items-start justify-between mb-2">
                                <div>
                                    ...
                                    <p class="text-sm text-muted-foreground dark:text-muted-foreground mt-1">
                                        {{ group.group_description || t('data_management.no_description') }}
                                    </p>
                                </div>
                                <div class="text-right">
                                    <div class="text-sm text-muted-foreground dark:text-muted-foreground">{{ t('data_management.machines') }}</div>
                        ...
                        <div>
                            <h5 class="text-sm font-medium mb-3">{{ t('data_management.targets') }}</h5>
                            <TargetDisplay
