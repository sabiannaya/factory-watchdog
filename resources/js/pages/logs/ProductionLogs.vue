...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    ...
    { title: t('logs.production_logs'), href: '/logs/production' },
]);
...
const exportToExcel = () => {
    ...
    window.location.href = `/logs/production/export?${params.toString()}`;
};
...
<template>
    <Head :title="t('logs.production_logs')" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-4 space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-semibold">{{ t('logs.production_logs_title') }}</h2>
                    <p class="text-sm text-muted-foreground">{{ t('logs.production_logs_description') }}</p>
                </div>
